...
export default function Login() {
  ...
  const [siteData, setSiteData] = useState({ name: t('welcome_back'), logo: '' })
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => router.push(data.redirectPath), 1000)
      } else {
        toast.error(data.message || 'Login failed')
      }
    } catch (err: any) {
      toast.error('Gagal terhubung ke server') // Keep one as is or change to 'Connection failed'
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="flex items-center justify-center min-vh-screen bg-black p-4 min-h-screen relative overflow-hidden group">
      ...
      {/* Background Decor */}
      <div className="absolute top-0 -left-20 w-80 h-80 bg-indigo-500/10 rounded-full blur-[120px] animate-pulse transition-all duration-[4s]"></div>
      <div className="absolute bottom-0 -right-20 w-80 h-80 bg-purple-500/10 rounded-full blur-[120px]"></div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 shadow-2xl rounded-[40px] p-10 max-w-md w-full relative z-10 backdrop-blur-xl bg-opacity-80">
        ...
        <div className="text-center mb-10 space-y-6">
           <div className="mx-auto w-24 h-24 flex items-center justify-center">
              {siteData.logo ? (
                ...
                <div className="w-16 h-16 bg-gradient-to-tr from-indigo-600 to-purple-600 rounded-2xl shadow-2xl flex items-center justify-center shadow-indigo-600/30">
           ...
           <div>
             ...
             <p className="text-zinc-500 font-bold uppercase tracking-widest text-[10px] mt-2 flex items-center justify-center gap-2">
                <ShieldCheck className="w-4 h-4 text-emerald-500" /> {t('secure_encryption')}
             </p>
           </div>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">{t('master_pass')}</label>
            <div className="relative group/input">
               <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-zinc-600 group-focus-within/input:text-indigo-400">
        ...
        <div className="mt-10 text-center space-y-4 text-zinc-600 text-xs font-bold uppercase tracking-tight">
           <Link href="/auth/register" className="group text-indigo-400 font-black flex items-center justify-center gap-2 uppercase tracking-widest text-[11px] hover:text-white transition-colors">
              {t('init_reg')}
